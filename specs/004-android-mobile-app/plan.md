...
- **Consumo Unívoco de Configuración:** `QrScannerViewModel.consumeScannedConfig()` para que el payload detectado se consuma como evento único y no reactive la navegación al regresar de la pantalla de transmisión (`RF-001`).
- **Inicialización Inmediata en Pairing:** `CameraStreamViewModel(initialConfig = config)` para arrancar la pantalla de transmisión en `ConnectionState.Pairing(config)` desde el fotograma inicial, suscribiendo las escuchas de mensajes antes de abrir los sockets WebSocket (`RF-001`).
...
- **Aislamiento de Ciclo de Vida y Detección de Desconexión:** Control del ciclo de vida del ejecutor en `QrScannerScreen`, observación de `ON_STOP`/`ON_DESTROY` en `CameraScreen` para notificar desconexión y liberar `CameraXCaptureAdapter`, guardián de transición activa `hasActiveSession` para prevenir rebotes prematuros a escáner, y monitoreo en `KtorClientStreamAdapter` para emitir `DisconnectRequest("SERVER_CLOSED")` ante caída del socket del Host (`RF-005`, `RNF-005`).
