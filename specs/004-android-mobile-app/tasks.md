...
- [x] **T03: Adaptador de captura de video con compresión JPEG y métricas en vivo (`CameraXCaptureAdapter`)**  
  *Requisitos cubiertos:* `RF-002`, `RNF-001`, `RNF-002` (AGENTS.md Regla 4)  
  *Hecho cuando:* Se configure CameraX a 1080p/720p, se compriman frames a JPEG de alta eficiencia en un hilo dedicado `CameraX-Worker`, se calculen los FPS de salida reales y la tasa de bits en Kbps, y se transmitan sobre `/ws/stream` sin penalizar el hilo de Compose.
- [x] **T04: Adaptador de telemetría y responder de Ping/Pong RTT (`AndroidTelemetryProvider` / `KtorClientStreamAdapter`)**  
  ...
  *Hecho cuando:* El cliente de red responda inmediatamente enviando `ProtocolMessage.Pong` ante paquetes `Ping` del Host, emita periódicamente telemetría con batería, estado térmico y métricas reales de FPS/bitrate, y capture el cierre del canal entrante de Ktor emitiendo `DisconnectRequest("SERVER_CLOSED")` ante caída o terminación del Host.
...
- [x] **T05: Pruebas unitarias para `CameraStreamViewModel` (TDD)**  
  ...
  *Hecho cuando:* Se validen con Turbine los estados de conexión (`CameraUiState`), la reducción automática a 30 FPS ante estado térmico `SEVERE`/`CRITICAL`, la emisión periódica de `ProtocolMessage.TelemetryPacket`, el despacho de `ProtocolMessage.CommandPacket` ante acciones locales de zoom y linterna, y la ejecución de `CameraCommand` tipados provenientes del Host.
- [x] **T06: Implementación de `CameraStreamViewModel`**  
  *Requisitos cubiertos:* `RF-001`, `RF-003`, `RF-004`, `RF-005`  
  *Hecho cuando:* El ViewModel integre los casos de uso compartidos, soporte `initialConfig: PairingConfig?` inicializando su estado en `Pairing(config)` desde el fotograma inicial cuando esté disponible (preservando `Disconnected` por defecto cuando es nulo), emita periódicamente `TelemetryPacket` al gateway, despache `CommandPacket` hacia el Host ante cambios locales de zoom o linterna, controle la linterna con validación previa de hardware (`hasFlashUnit()`) y apruebe el 100% de las pruebas de `T05`.
